...
class TestGoNoGoCommand:
    ...
    async def test_gonogo_calls_evaluate_and_sends_report(self) -> None:
        ...
        report = GateReport(
            ...
            criteria=[
                CriterionResult(
                    name="uptime_pct", passed=True, actual=99.5,
                    threshold=95.0, unit="%", critical=True,
                ),
                CriterionResult(
                    name="fill_rate_pct", passed=True, actual=98.0,
                    threshold=90.0, unit="%", critical=True,
                ),
            ],
            sandbox_days=7,
    ...
    async def test_gonogo_formats_verdict_emoji(self) -> None:
        ...
        report = GateReport(
            ...
            criteria=[
                CriterionResult(
                    name="max_drawdown_pct", passed=False, actual=5.0,
                    threshold=2.27, unit="%", critical=True,
                ),
            ],
            sandbox_days=7,
            evaluated_at=datetime.now(UTC),
