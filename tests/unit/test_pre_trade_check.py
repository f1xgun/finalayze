...
def _base_ctx(**overrides: object) -> CheckContext:
    """Return a CheckContext that passes all standard pre-trade checks."""
...
class TestSectorConcentration:
    ...
    def test_sector_concentration_below_limit_passes(self) -> None:
        ...
        result = checker.check(_base_ctx(sector_exposure_value=Decimal(30000), sector_id="us_tech"))
        assert result.passed

    def test_sector_concentration_at_limit_fails(self) -> None:
        checker = PreTradeChecker()
        # Existing sector exposure 36k + order 5k = 41k / 100k = 41% > 40%
        result = checker.check(_base_ctx(sector_exposure_value=Decimal(36000), sector_id="us_tech"))
    ...
    def test_sector_concentration_custom_cap(self) -> None:
        ...
        result = checker.check(_base_ctx(sector_exposure_value=Decimal(26000), sector_id="us_tech"))
        assert not result.passed
...
class TestCashReserve:
    ...
    def test_cash_reserve_below_20pct_fails(self) -> None:
        ...
        result = checker.check(_base_ctx(available_cash=Decimal(25000), order_value=Decimal(10000)))
        assert not result.passed
    ...
    def test_cash_reserve_exact_boundary(self) -> None:
        ...
        result = checker.check(_base_ctx(available_cash=Decimal(25000), order_value=Decimal(5000)))
        assert result.passed
    ...
    def test_cash_reserve_custom_threshold(self) -> None:
        ...
        result = checker.check(_base_ctx(available_cash=Decimal(35000), order_value=Decimal(10000)))
        assert not result.passed
