...
def fifo_match(
    ...
    Raises ``FifoMatchError`` on an oversell (a SELL with no matching open lot).
    Both lists are processed in acquisition/disposal-date order (stable FIFO).
    """
    open_lots: list[_OpenLot] = [_op_lot(b) for b in sorted(buys, key=lambda o: o.op_date)]
    results: list[RealizedResult] = []

    for sell in sorted(sells, key=lambda o: o.op_date):
        remaining_to_match = sell.quantity if sell.quantity is not None else Decimal(0)
        sell_price = sell.price_per_unit if sell.price_per_unit is not None else Decimal(0)
        ...
        while remaining_to_match > 0:
            ...
            if lot is None:
                msg = (
                    f"oversell: SELL of {sell.ticker} on {sell.op_date} exceeds known "
            ...
            buy_comm_share = (
                lot.commission_buy_remaining * take / lot.remaining_qty
                if lot.remaining_qty > 0
                else Decimal(0)
            )
            # sell commission apportioned across the whole SELL by qty
            sell_comm_share = (
                sell.commission * take / total_sell_qty if total_sell_qty > 0 else Decimal(0)
            )
            results.append(
                RealizedResult(
                    figi=lot.figi,
...
def open_lots_after_match(
    ...
    Raises ``FifoMatchError`` on oversell (same rule as ``fifo_match``).
    """
    working: list[_OpenLot] = [_op_lot(b) for b in sorted(buys, key=lambda o: o.op_date)]
    _consume(working, sells)
...
def _consume(working: list[_OpenLot], sells: list[Operation]) -> None:
    for sell in sorted(sells, key=lambda o: o.op_date):
        remaining_to_match = sell.quantity if sell.quantity is not None else Decimal(0)
        while remaining_to_match > 0:
